...
require(tcltk)
  ...
  if(bcp$fixed_times) {
    i = which(x$timestamp>bcp$time.gate[1]&x$timestamp<bcp$time.gate[2] ) 
    O$good = !O$good
    O$good[i] = TRUE
    }
    if(!bcp$fixed_times){ #remove all gating if choosing fixed times AMC Feb 1 2024
  ...
  if ( bcp$user.interaction  ) {
    ...
    if("sensordepth1" %in% names(O$plotdata)){
      if(length(which(!is.na(O$plotdata$sensordepth1[grange]))) > 5){
        ...
        mtext(2,text=depth.alts[1], col = "green", line=5.5)
      }
    }
    ...
    id2 = identify(O$plotdata$timestamp[grange], O$plotdata$depth[grange], labels = c(as.character(O$plotdata$timestamp[grange])),n = 1, pos = TRUE,offset=-2)
